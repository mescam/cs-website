...
%-------------------------------------------------
\begin{frame}{Terraform Core vs Provider – diagram}
  \centering
  \begin{tikzpicture}[
    ...
    \node[ext] (tf) {Pliki \texttt{.tf}\\(HCL)};
    \node[box, below=of tf] (core) {Terraform Core\\\scriptsize(plan, graf, state)};
    \node[ext, below left=0.9cm and 0.2cm of core] (prov1) {Provider AWS};
    \node[ext, below right=0.9cm and 0.2cm of core] (prov2) {Provider Azure};

    \node[ext, below=0.9cm of prov1] (aws) {AWS API};
    \node[ext, below=0.9cm of prov2] (az) {Azure API};

    \draw[arrow] (tf) -- node[right]{parse HCL} (core);

    \draw[arrow] (core.south) -- ++(-1.0,-0.3) |- node[left, xshift=-0.1cm]{\scriptsize RPC / JSON} (prov1.north);
    \draw[arrow] (core.south) -- ++(1.0,-0.3) |- node[right, xshift=0.1cm]{\scriptsize RPC / JSON} (prov2.north);

    \draw[arrow] (prov1) -- (aws);
    \draw[arrow] (prov2) -- (az);

    \node[above right=0.1cm and -0.2cm of core] {\scriptsize \texttt{terraform plan/apply}};
  \end{tikzpicture}

  \vspace{0.4cm}
  {\small
    Terraform Core \emph{tylko} liczy plan i woła providery,\\
